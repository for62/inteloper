...
(function (window, document, $) {
    ...
    $.extend(F, {
        ...
        defaults: {
            ...
            // HTML templates
            tpl: {
                wrap: '<div id="fancybox-wrap" class="fancybox-wrap" tabIndex="-1"><div id="fancybox-outer" class="fancybox-outer"><div id="fancybox-content" class="fancybox-inner"></div></div></div>',
                image: '<img id="fancybox-img" class="fancybox-image" src="{href}" alt="" />',
                iframe: '<iframe id="fancybox-frame{rnd}" class="fancybox-iframe" name="fancybox-frame{rnd}" frameborder="0" hspace="0"' + ($.browser.msie ? ' allowtransparency="true"' : '') + '></iframe>',
        ...
        _setDimension: function () {
            ...
            if (current.fitToView == true) {
                ...
                var _width = (($(window).width() - padding2 - 50) > (width + padding2)) ? (width + padding2) : ($(window).width() - padding2 - 50);
                var _height = $(window).height() - padding2 - 100;
                inner.width(_width);
                inner.height(_height);
                inner.css("overflow", "auto");
                inner.find("img").width(width).height(height);
                // inner.find("img").width(width).height(height).css("cursor","move");
                // var offsetX = inner.find("img").offset().top;
                // var offsetY = inner.find("img").offset().left;
                // $(".fancybox-nav").hide();
                // var _tempX, _tempY;
                // inner.unbind("mousedown");
                // inner.bind("mousedown", function(e){
                // 	_tempX = e.pageX;
                // 	console.log(_tempX);
                // 	_tempY = e.pageY;
                // 	// inner.bind("mousemove", function(ee){
                // 	// 	var _offsetX = offsetX - (ee.pageX - _tempX);
                // 	// 	var _offsetY = offsetY - (ee.pageY - _tempY);
                // 	// 	inner.find("img").offset({
                // 	// 		top: _offsetX > (width - _width) ? (width - _width) : _offsetX,
                // 	// 		left: _offsetY > (height - _height) ? (height - _height) : _offsetY
                // 	// 	});
                // 	// 	console.table([_offsetX, (width - _width)])
                // 	// 	// inner.find("img").scrollTop(ee.pageX - _tempX);
                // 	// 	// inner.find("img").scrollLeft(ee.pageY - _tempY);
                // 	// })
                // })
                // inner.unbind("mouseup");
                // inner.bind("mouseup", function(e){
                // 	var _offsetX = offsetX + (e.pageX - _tempX);
                // 	var _offsetY = offsetY + (e.pageY - _tempY);
